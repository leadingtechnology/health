...
class ApiService {
  ...
  // OpenAI integration
  Future<ApiResult<String>> askOpenAI(String prompt, {String? model}) async {
    if (!isAuthenticated) {
  ...
  Future<ApiResult<List<PlanConfig>>> getPlans() async {
    ...
    if (res.statusCode >= 200 && res.statusCode < 300) {
      try {
        ...
        if (data is List) {
          final plans = PlanConfigurations.fromApi(data);
          return ApiResult.success(plans);
        }
        return ApiResult.error('Invalid response type');
      } catch (e) {
        return ApiResult.error('Failed to parse response: $e');
      }
    } else {
